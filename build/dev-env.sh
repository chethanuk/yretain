...
minikube image load "yretain:${TAG}"

echo "[dev-env] creating yretain namespace"
kubectl create namespace yretain

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl get namespace yretain -n yretain"
...
kubectl create -f manifests/persistent-storage-no-pvc-deletion.yaml -n yretain

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment.apps/rfs-redisfailover-persistent-keep -n yretain"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
STATEFULSET_REPLICAS=$(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.spec.replicas}' -n yretain)
ATTEMPTS=0
until [[ ${STATEFULSET_REPLICAS} -eq $(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.status.readyReplicas}' -n yretain) ]] || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done

echo "[dev-env] installing yretain charts"
helm upgrade --install \
    yretain charts/yretain \
    --namespace yretain \
    --set ingress.host.name="yretain.${INGRESS_HOST}"

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment/yretain -n yretain"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
FastAPI available under: http://yretain.${INGRESS_HOST}/
