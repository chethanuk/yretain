...
POETRY_VERSION="${POETRY_VERSION:=1.4.0}"
if ! command -v "$POETRY_BINARY" &> /dev/null; then
  echo "[install] Poetry is not installed. Begin download and install."
  curl -sSL https://raw.githubusercontent.com/python-poetry/poetry/1.4.0/install-poetry.py | POETRY_HOME=$POETRY_HOME POETRY_VERSION=$POETRY_VERSION $PYTHON -
fi
...
Now you should access CLI script: $ yretain --help
Alternatively you can access CLI script via poetry run: $ poetry run yretain --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_YRETAIN_COMPLETE=source_bash yretain)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_YRETAIN_COMPLETE=source_zsh yretain)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_YRETAIN_COMPLETE=source_fish yretain)' >> ~/.config/fish/completions/yretain.fish
EOF